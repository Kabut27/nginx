...
# ---- ensure nginx is installed, offer latest mainline upgrade ----
install_nginx_mainline() {
  info "Adding the official nginx.org apt repo and installing latest mainline nginx..."
  apt-get update -y
  ...
  # nginx.org packages load /etc/nginx/conf.d/*.conf by default, not sites-available/
  # sites-enabled — make sure our site still gets loaded either way.
  ...
  # avoid the stock conf.d/default.conf clashing with our site on 80/443
  if [[ -f /etc/nginx/conf.d/default.conf ]]; then
    mv /etc/nginx/conf.d/default.conf /etc/nginx/conf.d/default.conf.disabled
    info "Disabled the stock conf.d/default.conf to avoid a port clash."
  fi

  systemctl enable nginx
  systemctl restart nginx
  ok "nginx ready: $(nginx -v 2>&1 | sed 's/nginx version: //')"
}

if command -v nginx >/dev/null 2>&1; then
  info "nginx is installed: $(nginx -v 2>&1 | sed 's/nginx version: //')"
  info "Distro-packaged nginx can lag behind — that's what caused any 'unknown directive'"
  info "errors on older setups (e.g. missing http2 support)."
  if ask_yes_no "Upgrade to the latest mainline nginx from the official nginx.org repo?" "n"; then
    install_nginx_mainline
  fi
else
  err "nginx is not installed on this server."
  if ask_yes_no "Install the latest mainline nginx now (official nginx.org repo)?" "y"; then
    install_nginx_mainline
  else
    apt-get update -y
    apt-get install -y nginx
    ...
    ok "nginx installed from the distro repo: $(nginx -v 2>&1 | sed 's/nginx version: //')"
  fi
fi
...
ensure_ssl_tuning() {
  ...
  if ! grep -q "ssl_session_cache" /etc/nginx/nginx.conf 2>/dev/null; then
    sed -i '/^http {/a\    ssl_session_cache shared:SSL:10m;' /etc/nginx/nginx.conf
    added=1
  fi
  if ! grep -q "ssl_session_timeout" /etc/nginx/nginx.conf 2>/dev/null; then
    sed -i '/^http {/a\    ssl_session_timeout 1h;' /etc/nginx/nginx.conf
    added=1
  fi
  if ! grep -q "ssl_session_tickets" /etc/nginx/nginx.conf 2>/dev/null; then
    sed -i '/^http {/a\    ssl_session_tickets off;' /etc/nginx/nginx.conf
    added=1
  fi
  if ! grep -q "ssl_protocols" /etc/nginx/nginx.conf 2>/dev/null; then
    sed -i '/^http {/a\    ssl_protocols TLSv1.2 TLSv1.3;' /etc/nginx/nginx.conf
    added=1
  else
    info "ssl_protocols already set elsewhere in nginx.conf, leaving it as-is."
  fi
  if ! grep -q "ssl_prefer_server_ciphers" /etc/nginx/nginx.conf 2>/dev/null; then
    sed -i '/^http {/a\    ssl_prefer_server_ciphers off;' /etc/nginx/nginx.conf
    added=1
  else
    info "ssl_prefer_server_ciphers already set elsewhere in nginx.conf, leaving it as-is."
  fi

  if [[ "$added" == "1" ]]; then
    ok "SSL session/protocol tuning applied (only missing directives were added)."
  else
    info "SSL tuning already fully present in nginx.conf."
  fi
}
